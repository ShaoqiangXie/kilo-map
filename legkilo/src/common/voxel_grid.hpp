...
class VoxelGrid {
   public:
    ...
    void filter(const CloudPtr& cloud_in, CloudPtr& cloud_out) const {
        ...
        // Both filtering implementations append directly to points for speed.
        // Keep the PCL organization metadata in sync: transformPointCloud()
        // passes width to PointCloud::assign(), where a zero width causes SIGFPE.
        pcl_utils::setUnorganizedCloudMetadata(cloud_out, cloud_in->is_dense);
    }
