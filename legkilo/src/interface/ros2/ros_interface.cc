...
std::shared_ptr<rclcpp::Node> createSubscriptionNode(const std::shared_ptr<rclcpp::Node>& parent,
    ...
    options.context(parent->get_node_base_interface()->get_context());
    options.use_global_arguments(false);
    options.enable_rosout(false);
    return std::make_shared<rclcpp::Node>(node_name, parent->get_namespace(), options);
}

rclcpp::QoS createReliableSubscriptionQos(const size_t depth) {
...
RosInterface::RosInterface(const std::shared_ptr<rclcpp::Node>& node, ViewerSlamInterface* viewer_interface)
    : node_(node), viewer_interface_(viewer_interface) {
    LOG(INFO) << "Ros2 Interface is being Constructed";
    tf_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*node_);
}

RosInterface::~RosInterface() {
    LOG(INFO) << "Ros2 Interface is being Destructed";
    this->stop();
}
...
bool RosInterface::initParamAndReset(const std::string& config_file) {
    ...
    livox_driver_version_ = yaml_helper.get<int>("livox_driver_version", 2);
    if (livox_driver_version_ != 1 && livox_driver_version_ != 2) {
        LOG(WARNING) << "Unsupported livox_driver_version=" << livox_driver_version_ << "; using version 2";
        livox_driver_version_ = 2;
    }
...
void RosInterface::createPublishers() {
    const auto state_qos = rclcpp::QoS(rclcpp::KeepLast(100));
    const auto cloud_qos = rclcpp::QoS(rclcpp::KeepLast(10));

    pub_odom_world_ = node_->create_publisher<ros_compat::OdometryMsg>("/Odometry", state_qos);
    pub_path_ = node_->create_publisher<ros_compat::PathMsg>("/path", state_qos);
    pub_pointcloud_world_ = node_->create_publisher<ros_compat::PointCloud2Msg>("/cloud_registered", cloud_qos);
    pub_pointcloud_body_ = node_->create_publisher<ros_compat::PointCloud2Msg>("/cloud_registered_body", cloud_qos);
    if (pub_joint_tf_enable_) {
        pub_joint_state_ = node_->create_publisher<ros_compat::JointStateMsg>("/joint_states", state_qos);
    }
}
...
void RosInterface::subscribeLidar() {
    ...
    const auto lidar_qos = createReliableSubscriptionQos(100);
    if (lidar_processing_->getLidarType() == common::LidarType::LIVOX) {
        if (livox_driver_version_ == 1) {
            sub_livox_v1_lidar_raw_ = lidar_node_->create_subscription<ros_compat::LivoxCustomMsgV1>(
                options::kLidarTopic, lidar_qos, std::bind(&RosInterface::livoxV1LidarCallBack, this, _1));
        } else {
        ...
        sub_lidar_raw_ = lidar_node_->create_subscription<ros_compat::PointCloud2Msg>(
            options::kLidarTopic, lidar_qos, std::bind(&RosInterface::lidarCallBack, this, _1));
    }
    lidar_executor_->add_node(lidar_node_);
    lidar_thread_ = std::make_unique<std::thread>(&RosInterface::lidarLoop, this);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
}

void RosInterface::subscribeImu() {
    if (!options::useImu()) return;
    ...
    const auto imu_qos = createReliableSubscriptionQos(2048);
...
void RosInterface::subscribeKinematicImu() {
    if (!options::useKinematics()) return;
    ...
    const auto kinematic_qos = createReliableSubscriptionQos(2048);
    sub_kinematic_raw_ = kinematic_node_->create_subscription<ros_compat::HighStateMsg>(
        options::kKinematicTopic, kinematic_qos, std::bind(&RosInterface::kinematicImuCallBack, this, _1));
...
void RosInterface::lidarLoop() {
    ...
    if (lidar_executor_) { lidar_executor_->spin(); }
}

void RosInterface::imuLoop() {
    LOG(INFO) << "IMU Loop Begin";
    if (imu_executor_) { imu_executor_->spin(); }
}

void RosInterface::kinematicImuLoop() {
    LOG(INFO) << "Kinematic Loop Begin";
    if (kinematic_executor_) { kinematic_executor_->spin(); }
}
...
void RosInterface::stopSlam() {
    ...
    sub_livox_v1_lidar_raw_.reset();
    sub_livox_lidar_raw_.reset();
    sub_imu_raw_.reset();
    sub_kinematic_raw_.reset();

    if (lidar_executor_) { lidar_executor_->cancel(); }
    if (imu_executor_) { imu_executor_->cancel(); }
    if (kinematic_executor_) { kinematic_executor_->cancel(); }
    ...
    if (lidar_executor_ && lidar_node_) { lidar_executor_->remove_node(lidar_node_); }
    if (imu_executor_ && imu_node_) { imu_executor_->remove_node(imu_node_); }
    if (kinematic_executor_ && kinematic_node_) { kinematic_executor_->remove_node(kinematic_node_); }

    lidar_executor_.reset();
    imu_executor_.reset();
    kinematic_executor_.reset();
    lidar_node_.reset();
    imu_node_.reset();
    kinematic_node_.reset();
...
void RosInterface::lidarCallBack(ros_compat::PointCloud2MsgConstPtr msg) {
...
void RosInterface::livoxLidarCallBack(ros_compat::LivoxCustomMsgConstPtr msg) {
    common::LidarScan lidar_scan;
...
void RosInterface::livoxV1LidarCallBack(ros_compat::LivoxCustomMsgV1ConstPtr msg) {
    auto converted = std::make_shared<ros_compat::LivoxCustomMsg>();
    converted->header = msg->header;
    converted->timebase = msg->timebase;
    converted->point_num = msg->point_num;
    converted->lidar_id = msg->lidar_id;
    converted->rsvd = msg->rsvd;
    converted->points.resize(msg->points.size());

    for (size_t i = 0; i < msg->points.size(); ++i) {
        const auto& src = msg->points[i];
        auto& dst = converted->points[i];
...
void RosInterface::kinematicImuCallBack(ros_compat::HighStateMsgConstPtr msg) {
    ...
    if (pub_joint_tf_enable_ && pub_joint_state_) {
...
void RosInterface::publishOdomTFPath(const double end_time, const Eigen::Vector3d& pos, const Eigen::Matrix3d& rot) {
    ...
    transform.transform.rotation.w = q_eigen.w();
    transform.transform.rotation.x = q_eigen.x();
    transform.transform.rotation.y = q_eigen.y();
    ...
    path_world_.poses.push_back(pose_path);
    path_world_.header.stamp = odom_world.header.stamp;
...
void RosInterface::run() {
    if (slam_stopped_.load(std::memory_order_acquire)) return;//  // 使用 acquire 内存序读取原子变量；如果已停止，则不再处理本次数据。
    if (!this->syncPackage()) return;//组装数据包，如果没有同步成功，则返回。

    ProcessResult process_result = kilo_->process(measure_);// 将同步后的传感器数据交给 KILO 前端处理。
    LOG_EVERY_N(INFO, 20) << process_result;//    // 每处理 20 次输出一次前端处理结果，避免日志过于频繁。
    if (!process_result.valid) return;//    // 当前帧处理无效时，不执行后端更新和 ROS 数据发布。

    const double end_time = measure_.lidar_scan_.lidar_end_time_;//    // 获取当前激光雷达扫描的结束时间，作为当前帧的时间戳。
    ...
    CloudPtr cloud_world = process_result.cloud_world;//    // 获取已经转换到世界坐标系下的当前帧点云。
    this->publishOdomTFPath(end_time, kilo_->getPosImu(), kilo_->getRotImu());//    // 发布当前位姿对应的里程计、TF 坐标变换和轨迹。
    this->publishPointcloudWorld(end_time, cloud_world);//    // 发布世界坐标系下的点云。
}
